...
public class CarNetConsoleHandler extends AbstractConsoleCommandExtension {

    private static final String CMD_LOGIN = "login";
    private static final String CMD_REFRESH = "refresh";

    private final Logger logger = LoggerFactory.getLogger(CarNetConsoleHandler.class);

    @Reference(cardinality = ReferenceCardinality.OPTIONAL)
    private @Nullable ClientBuilder injectedClientBuilder;

    @Activate
    public CarNetConsoleHandler() {
    ...
    public List<String> getUsages() {
        return Arrays.asList(
                buildCommandUsage(CMD_LOGIN + " [<email>] [<password>]",
                        "Logs into the account with the provided credentials and retrieves the access token."),
                buildCommandUsage(CMD_REFRESH, "Refresh access token."));
    }
